...
jQuery(window).on('load', function() {
    ...
    function setPage(newPage) {
        ...
        PAGE_LINK.children().first().css('opacity', 0);
        window.scroll(0, 0);
        updateNavigation();
        ...
        PAGE_LINK.html('<img src="'+ALL_PAGES[currentPage]+'" alt="'+pageLabel+'" style="opacity:0">');
    ...
    function finishImage(img) {
        if (!IS_FULLSCREEN) {
            // Set a fixed height on the wrapper so it doesn't collapse the next time the page changes.
            var ratio = CONTAINER_WIDTH / img.naturalWidth;
